...
def train(
    model, optimizer, scheduler, epochs, trainloader, valloader, trial=None, plot=False
):
...
def objective(
    ...
    if first_trial == True:
        ...
        final_mlp_style = trial.suggest_categorical(
        ...
        # Let's always use the same optimizer and scheduler
        optimizer = "AdamW"
        scheduler = "cosine"

    print(
        f"lr: {lr}, weight_decay: {weight_decay}, epochs: {epochs}, dim: {dim}, depth: {depth}, heads: {heads}, attn_dropout: {attn_dropout}, ff_dropout: {ff_dropout}, mlp_hidden_mults: {mlp_hidden_mults}, final_mlp_style: {final_mlp_style}, optimizer: {optimizer}, scheduler: {scheduler}"
    )
    ...
    valid_rmse = train(
        model, optimizer, scheduler, epochs, trainloader, validloader, trial
    )
